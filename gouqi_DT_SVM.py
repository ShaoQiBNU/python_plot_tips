...
def WZ_result(X1, y1, X, y, wz):
    ...
    p0, = plt.plot([0,8], [0,8], '--', color='black', label='line',linewidth = 1.0)
    
    color = ['limegreen', 'mediumslateblue', 'cyan', 'gold']
    marker = ['*', 'o', 'd', '<']
    
    p1 = plt.scatter(y1[0:12], predict[0:12], c=color[0], marker=marker[0], label='NQ1', s=280, edgecolors= 'black')
    p2 = plt.scatter(y1[12:24], predict[12:24], c=color[1], marker=marker[1], label='NQ5', s=150, edgecolors= 'black')
    p3 = plt.scatter(y1[24:36], predict[24:36], c=color[2], marker=marker[2], label='NQ7', s=180, edgecolors= 'black')
    p4 = plt.scatter(y1[36:48], predict[36:48], c=color[3], marker=marker[3], label='NNQ9', s=180, edgecolors= 'black')
...
predict = WZ_result(X1, y1, X, y, 'SGFD_DT_SVM')

df_result = pd.DataFrame()
df_result['predict'] = predict
df_result['HT'] = y1
df_result['label'] = df['label']
df_result.to_csv("Coef_SVM_HT_SNV.csv", index=False)
